...
import type { NumberArray } from '../index.ts';
import { PolynomialRegression } from '../index.ts';
...
function assertCoefficientsAndPowers(
  ...
  for (let i = 0; i < expectedCs.length; ++i) {
    const expected = expectedCs[i] ?? 0;

    expect(result.coefficients[i]).toBeCloseTo(expected, 10e-6);
...
test('basic linear test', () => {
  ...
  for (let i = 0; i < size; i++) {
    const xi = x[i] ?? 0;
    const yi = y[i] ?? 0;
    difference += Math.abs(regression.predict(xi) - yi);
  }
...
test('degree 2', () => {
  ...
  expect(result.toString(4)).toBe('f(x) = 0.1785 * x^2 - 0.1925 * x + 0.8505');
  expect(result.toLaTeX(2)).toBe('f(x) = 0.18x^{2} - 0.19x + 0.85');
});

test('degree 2 typed array', () => {
...
test('We should get the same result using numeric degree', () => {
  const x = new Float64Array([-4, 4, 2, 3, 1, 8, 5, 7]);
...
test('white noise regression', () => {
  ...
  for (let i = 0; i < size; i++) {
    const xi = x[i] ?? 0;
    const yi = y[i] ?? 0;
    newY.push(yi - regression.predict(xi));
  }
